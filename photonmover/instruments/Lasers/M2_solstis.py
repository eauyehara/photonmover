...
class M2_Solstis(Instrument, TunableLaser):
    ...
    def initialize(self):
        ...
        try:
            ...
            print(
                'M2_Solstis: cannot open socket connection to %s:%d' %
            ...
            if json_reply['message']['transmission_id'][0] == 1 and json_reply['message']['parameters']['status'] == 'ok':
                print(
                    'M2_Solstis: successfully started link to %s:%d as %s' %
    ...
    def set_wavelength(self, wavelength):
        ...
        while status != 3:
            ...
            if status == -1:    # just started
                ...
                # idle happened. If wavelength is close enough collect data,
                # else reset wavelength. Stop tuning.

                print('ERROR M2_Solstis: idle. software inactive!')

                measured_wavelength = self.get_wavelength()

                if np.abs((measured_wavelength - wavelength)
                          ) > self.wavelength_tolerance:
                    ...
                    status = 3
                    print(
                        'Despite idle wavelength is close and data will be acquired. Moving forward with data acquisition')
